...
function generatePassword(options) {
	...
	for (let i = 0; i < options.length; i++) {
		...
		options.hasUppercase ? uppercase : uppercase = "";
		options.hasLowercase ? lowercase : lowercase = "";
		options.hasNumbers ? numbers : numbers = "";
		options.hasSymbols ? symbols : symbols = "";

		let char = numbers + lowercase + uppercase + symbols;

		// let mainChar = char[Math.floor(Math.random() * options.length)];

		console.log(char)

		// Choose one of the generated characters at random
		// let char = Math.random() < 0.35
		// 	? numbers
		// 	: Math.random() < 0.5
		// 		? lowercase
		// 		: Math.random() < 0.75
		...
		let mainPassword = password[Math.floor(Math.random() * options.length)];
