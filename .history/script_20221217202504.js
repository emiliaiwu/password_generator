...
const passwordGenerator1 = () => {
	...
	generateBtn.addEventListener('click', () => {
		...
		resultEl.textContent = generatePassword(length, hasUppercase, hasLowercase);
	});



	const generatePassword = (options) => {
		const selectedChar = options.hasUppercase + options.hasLowercase
			+ options.hasNumbers + options.hasSymbols;
